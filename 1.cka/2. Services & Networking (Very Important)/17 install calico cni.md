...
> Make sure all primary commponents is running
...
> In this task, we are now using **Kubernetes v1.34**. The cluster eventually reaches a **Ready** state, and there is no prolonged **NotReady** status. **CoreDNS** is also in a **Running** state.
> 
> In the older version, **CoreDNS remained in Pending**, and the **node status stayed NotReady**.

- ❌ **Older version behavior**
    
    - CoreDNS stuck in **Pending**
        
    - Nodes stayed **NotReady**
        
    - Commonly caused by:
        
        - CNI plugin not initializing
        - Insufficient node resources
        - kubelet or control-plane timing issues
        - Version-specific bugs fixed in newer releases
...
```
in downloaded custom-resource , you 'll find this below yaml of kind Installation 
just edit cidr range in that only then apply 
for cidr value , check podsubnet value  in kubeadm-config cm in kube-system ns or
check podcidr in node , just change it to x.x.x.x/24 to x.x.x.x/16
```
