...
Do the following:
    ...
    Examine the existing Deployment at /opt/course/16/cleaner.yaml
    Save your changes to /opt/course/16/cleaner-new.yaml
    Add a sidecar container named logger-con with:
        Image: public.ecr.aws/docker/library/busybox:latest
        Mount the same volume as cleaner-con
        Use tail -F command to stream /var/log/cleaner.log to stdout
    Apply the updated Deployment to make it running
    Verify the logs are accessible via kubectl logs
    Check the logs to find information about the missing data incidents
